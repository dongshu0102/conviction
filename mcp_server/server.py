...
async def get_rate_signals(neutral_real_rate: float | None = None, target_inflation: float | None = None) -> str:
    ...
    if target_inflation is not None:
        params["target_inflation"] = target_inflation
    return await _request("GET", "/companies/rate-signals", params=params)
...
async def get_institutional_portfolio(filer: str, limit: int = 50) -> str:
    ...
    confirmed real case: searching "Vanguard" resolves to "Vanguard
    Capital Management LLC", one of several genuinely distinct,
    unrelated SEC-registered Vanguard entities), say so explicitly
    rather than silently presenting it as if it were the entity the
    person meant."""
