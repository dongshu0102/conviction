...
def get_ticker_map_repository() -> CusipTickerMapRepository:
    return SqlAlchemyCusipTickerMapRepository()


def _to_schema(h, ticker_map: dict) -> InstitutionalHoldingSchema:
    mapping = ticker_map.get(h.cusip)
...
def get_holders(
    ...
    ticker_map_repo: CusipTickerMapRepository = Depends(get_ticker_map_repository),
) -> InstitutionalHoldersResponseSchema:
    ...
    ticker_map = ticker_map_repo.get_many([h.cusip for h in result.holders])
    ...
    return InstitutionalHoldersResponseSchema(
        ...
        holders=[_to_schema(h, ticker_map) for h in result.holders],
...
def get_portfolio(
    ...
    ticker_map = ticker_map_repo.get_many([h.cusip for h in result.holdings])

    return InstitutionalPortfolioResponseSchema(
        filer_query=result.filer_query, filer_name=result.filer_name,
        period_of_report=result.period_of_report,
        holdings=[_to_schema(h, ticker_map) for h in result.holdings],
        source=result.source, source_note=_SOURCE_NOTES[result.source],
    )
...
def get_position_changes(
    ...
    ticker_map = ticker_map_repo.get_many([c.cusip for c in result.changes])
    ...
    return PositionChangesResponseSchema(
        ...
        changes=[
            PositionChangeSchema(
                cusip=c.cusip,
                ticker=(ticker_map[c.cusip].ticker if c.cusip in ticker_map else None),
