...
from src.domain.services.capital_flow_math import next_13f_deadline
...
_TOOLS = [
    ...
    ToolDefinition(
        ...
        "across up to 5 real sources: INSIDER (executives/directors "
        ...
        "'$1,000,001 - $5,000,000', never an exact figure; may also "
        "carry a real 'late filing' flag when disclosed more than the "
        "STOCK Act's 45-day deadline after the trade), VOLUME (a "
        "ticker's real daily trading volume spiking to several times "
        "its own recent average — direction is always UNKNOWN here, "
        "since a volume spike alone doesn't say which way money moved, "
        "only that something unusual happened), and MACRO "
...
class ChatWithAgentUseCase:
    ...
    def _dispatch(self, tool_name: str, tool_input: dict):
                ...
                "events": [
                    {
                        ...
                        "detected_at": e.detected_at.isoformat(), "is_late_filing": e.is_late_filing,
                    }
                    for e in events
                ],
                "note": (
        ...
        if tool_name == "get_next_13f_deadline":
            today = date.today()
            deadline = next_13f_deadline(today)
            return {
                "next_deadline": deadline.isoformat() if deadline else None,
