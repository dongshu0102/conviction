...
class IngestForm13FQuarterUseCase:
    ...
    def execute(
        self, period_label: str, period_of_report: date, replace_existing: bool = True,
        ...
        (e.g. date(2026, 3, 31)) used to scope delete_period and to
        confirm which rows this run is responsible for — the two are
        independent because SEC's own file-naming convention isn't
        always a clean single quarter (some archives span 3 calendar
        months that don't align to a quarter boundary, e.g.
        "01mar2026-31may2026")."""
        ...
        deleted = self._repository.delete_period(period_of_report) if replace_existing else 0
        inserted = self._repository.bulk_save(holdings)
        ...
        return IngestForm13FQuarterResult(
            ...
            holdings_deleted=deleted, holdings_inserted=inserted,
        )
